...
const UpdateDetector = () => {
  ...
  const [phase, setPhase] = useState<'detecting' | 'loading' | 'ready'>('detecting');
  ...
  // Animate progress when update detected
  useEffect(() => {
    ...
    stages.forEach((stage) => {
      ...
      timeout = setTimeout(() => setProgress(stage.target), delay);
    });

    // Mark ready then reload
    setTimeout(() => {
      setPhase('ready');
