...
const CreditCostsAccordion = () => {
  ...
  const getActualCost = (item: CostRow): number | string => {
    if (item.costKey.startsWith('_')) return item.fallbackCost;
    ...
    <section>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-heading font-semibold">Coût des actions</h2>
        </div>
        {promoCount > 0 && (
          <Badge className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 text-[10px] gap-1 font-bold">
      ...
      <Accordion type="multiple" className="space-y-2">
        {filtered.map(cat => {
          ...
          return (
            <AccordionItem key={cat.id} value={cat.id} className="border rounded-2xl overflow-hidden border-border/50 bg-card/60 backdrop-blur-sm">
              <AccordionTrigger className="text-[13px] font-medium px-4 py-3 hover:no-underline hover:bg-muted/20">
                <span className="flex items-center gap-2.5">
                  <span className="text-base">{cat.emoji}</span>
                  <span className="font-heading">{cat.title}</span>
                  {hasPromo && (
                    <Badge className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 text-[8px] px-1.5 py-0 h-4 font-bold">
                      PROMO
                    </Badge>
                  )}
                </span>
                  ...
                  {cat.items.map((item, j) => {
                    ...
                    return (
                      <div
                        key={j}
                        className={cn(
                          "flex items-center justify-between py-2.5 px-2.5 rounded-xl transition-colors",
                          hasDiscount
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          {hasDiscount && discount > 0 && (
                            <Badge className="bg-emerald-500 text-white text-[7px] px-1 py-0 h-3.5 font-bold">
                              -{discount}%
                            </Badge>
                          )}
                        </div>
