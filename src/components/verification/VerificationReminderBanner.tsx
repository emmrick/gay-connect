...
const VerificationReminderBanner = () => {
  ...
  if (isLoading || isVerificationComplete || isVerificationPending || !showBanner) {
    return null;
  }

  // For re-verifications: show a soft reminder (no purge countdown)
  if (isReVerification) {
    return (
      <>
        <div className="fixed top-0 left-0 right-0 z-50 px-4 py-2 bg-primary text-primary-foreground">
          <div className="max-w-screen-lg mx-auto flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <Clock className="w-4 h-4 flex-shrink-0" />
              <p className="text-sm truncate">
                <span className="font-bold">🔄 Re-vérification annuelle requise</span>
                {' '}— Merci de soumettre tes documents
              </p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <Button 
                size="sm" 
                variant="outline"
                className="h-7 text-xs bg-transparent border-current text-current hover:bg-white/20"
        ...
        <div className="h-10" />
        <IdentityVerificationDialog open={showDialog} onOpenChange={setShowDialog} />
      </>
    );
  }

  // First-time verification: show purge countdown
